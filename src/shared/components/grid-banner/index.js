...
const INITIAL_NUMBER_OF_IMAGES = 26;
const FIRST_INDEX = 0;
...
export default class GridBanner extends Component {
  ...
  state = {
    number: INITIAL_NUMBER_OF_IMAGES
  };
    ...
    if (item) {
      return (
        <div className={styles.mobileContainer} onClick={this.rearrange}>
          <FadingImage url={item.small_url} />
        </div>
      );
    }
  }
  ...
  render() {
    ...
    const processedItems = shuffle(items)
      .filter(
        x => Boolean(x.original_width) && x.original_width > x.original_height
      )
    ...
    const images = processedItems.map((item, i) => {
      ...
      return (
        <Link
          to={{ pathname: `/media/${item.id}`, query: { type: item.type } }}
      ...
      <div>
        ...
        <div className={styles.container}>
          ...
          <div className={styles.rearrange}>
            <div onClick={this.removeImage}>Remove one image</div>
            <div onClick={this.rearrange}>Click to rearrange</div>
            <div onClick={this.addImage}>Add one image</div>
          </div>
        </div>
      </div>
    );
